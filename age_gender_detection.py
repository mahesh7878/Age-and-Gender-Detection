...
if not os.path.exists(ageModel):
    missing_models.append(ageModel)
if not os.path.exists(genderModel):
    missing_models.append(genderModel)
if not os.path.exists(faceModel):
    missing_models.append(faceModel)
...
if args.device == "cpu":
    ...
    try:
        ...
        print("Using GPU device (CUDA)")
    except Exception as e:
        print(f"[WARNING] GPU/CUDA not available: {str(e)[:80]}")
...
if not cap.isOpened():
    print("[ERROR] Cannot open video/camera source")
    print("If using webcam, check if camera is connected and not in use")
    exit(1)
